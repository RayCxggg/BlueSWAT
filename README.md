...
```
git clone --recursive https://github.com/RayCxggg/BlueSWAT.git
```
...
The eBPF firewall is ported to both stacks and lives next to each stack's BLE
controller/host:

- Zephyr: `ZephyrOS/zephyr/firewall/`
- Mynewt/NimBLE: `Mynewt/repos/apache-mynewt-nimble/firewall/`

Each port shares the same FSM core (`firewall/core/`, `firewall/include/`) and
the `libebpf/` interpreter, but keeps its stack-specific hooks separate.

### Policies

eBPF C sources are compiled to bytecode headers via `firewall/policy/compile.sh`.

- Zephyr ships 10 policies in `firewall/policy/ebpf_C_code/`
  (`conn_chan_hop`, `conn_chan_map`, `dc_nesn`, `llcp_conn_param_req`,
  `llcp_len_req`, `lll_interval`, `scan_rsp_len`, `smp_ident_check`,
  `spi_acl_len`, `spi_evt_len`).
- Mynewt ships 3 reference specifications in
  `firewall/policy/specification/eBPF_C_code/` (`conn_chan_map`,
  `key_entropy_downgrade`, `keysize_confusion`).

Generated bytecode headers are written to the matching `policy/.../include/`
directory. CRLF will break the script — keep line endings as LF.

### Runtime patch install (OTA)

Policies can also be installed at runtime instead of being baked into the
image. The runtime install path runs a structural eBPF verifier before the
program is loaded. On Mynewt, patches are delivered over a vendor GATT service
under `nimble/host/services/firewall/`.

### Host-side tests

Each port has a host-side harness that exercises the FSM and policy cache off
the device:

```
# Zephyr
make -C ZephyrOS/zephyr/firewall/tests

# Mynewt
make -C Mynewt/repos/apache-mynewt-nimble/firewall/tests
```
